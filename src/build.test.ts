...
describe('build', () => {
    describe('formatGigs', () => {
        ...
        it('wraps venue in link if venueUrl is present', () => {
            const concerts: Concert[] = [
                { date: new Date(2023, 11, 15), artist: 'Artist A', venue: 'Venue A', venueUrl: 'http://example.com' }
            ];
            const html = formatGigs(concerts);
    ...
    describe('build execution', () => {
        it('fetches data, generates html, and writes to file', async () => {
            ...
            vi.mocked(scraper.parseConcerts).mockReturnValue([
                { date: new Date(2023, 10, 1), artist: 'Test Artist', venue: 'Test Venue' }
            ]);
            vi.mocked(fs.readFile).mockResolvedValue('<div>{{CONTENT}}</div><footer>{{UPDATED_AT}}</footer>');
